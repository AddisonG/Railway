...
public class TrackTest {
	...
	public void testInitialState() {
		...
		notSections.add(new Section(9, new JunctionBranch(notJunctions.get(0), Branch.FACING),
				new JunctionBranch(notJunctions.get(1), Branch.FACING)));
		...
		Assert.assertEquals(expectedSection, track.getTrackSection(notJunctions.get(0), Branch.FACING));
	...
	public void testInvalidAddition() {
		...
		sections.add(new Section(9, new JunctionBranch(junctions.get(0), Branch.FACING),
				new JunctionBranch(junctions.get(1), Branch.NORMAL)));
		sections.add(new Section(20, new JunctionBranch(junctions.get(2), Branch.REVERSE),
				new JunctionBranch(junctions.get(0), Branch.FACING)));
	...
	public void testValidAdditions() {
		...
		List<Section> sections = new ArrayList<>();
		sections.add(new Section(9, new JunctionBranch(junctions.get(0), Branch.FACING),
				new JunctionBranch(junctions.get(1), Branch.FACING)));
		sections.add(new Section(20, new JunctionBranch(junctions.get(1), Branch.NORMAL),
				new JunctionBranch(junctions.get(2), Branch.FACING)));
		sections.add(new Section(6, new JunctionBranch(junctions.get(1), Branch.REVERSE),
				new JunctionBranch(junctions.get(3), Branch.NORMAL)));
		sections.add(new Section(80, new JunctionBranch(junctions.get(3), Branch.FACING),
				new JunctionBranch(junctions.get(4), Branch.NORMAL)));
		sections.add(new Section(10, new JunctionBranch(junctions.get(5), Branch.NORMAL),
				new JunctionBranch(junctions.get(6), Branch.REVERSE)));
				
		// sections that won't be in the track
		List<Section> notSections = new ArrayList<>();
		notSections.add(new Section(5, new JunctionBranch(junctions.get(0), Branch.NORMAL),
				new JunctionBranch(junctions.get(5), Branch.REVERSE)));
		...
		// check that there is no section connected to a junction that is not
		// on the track.
		Assert.assertEquals(null, track.getTrackSection(notJunctions.get(0), Branch.FACING));
		
		// check that there is no section connected to a junction that is on the
		// track, but doesn't have a particular branch.
		Assert.assertEquals(null, track.getTrackSection(junctions.get(0), Branch.REVERSE));
		...
		for (Section section : sections) {
			for (JunctionBranch endPoint : section.getEndPoints()) {
				Assert.assertEquals(section, track.getTrackSection(endPoint.getJunction(), endPoint.getBranch()));
			}
		}
		...
		for (Section section : track) {
			Assert.assertFalse("Duplicate section detected", actualSections.contains(section));
